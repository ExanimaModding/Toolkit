use super::{input::InputCollector, painter, utils};
...
use windows::Win32::{
	...
	UI::WindowsAndMessaging::{
		GetClientRect, GetGUIThreadInfo, GUITHREADINFO, GUITHREADINFO_FLAGS, WM_SIZING,
	},
};
...
impl<T> OpenGLApp<T> {
	...
	pub fn try_lock_state(&self) -> Option<MappedMutexGuard<'_, parking_lot::RawMutex, T>> {
		let data = self.data.lock();
		if data.is_none() {
			return None;
		}

		Some(MutexGuard::map(data, |app| {
			&mut app.as_mut().unwrap().state
		}))
	}
...
impl<T> OpenGLApp<T> {
	...
	pub fn render(&self, hdc: HDC) {
		unsafe {
			...
			let clipped_shapes = this.ctx.tessellate(output.shapes, 1.0);
