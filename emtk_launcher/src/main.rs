...
pub fn launch(path: &Path) -> Result<()> {
	let exanima_exe = path
		.join(emtk_core::Instance::BINARY)
		.canonicalize()
		.map_err(Error::msg(
	...
	let emtk_framework_dll = env::current_exe()
		.map_err(Error::msg("failed to find path to current executable"))?
		.with_file_name("emtk_framework.dll")
		.display()
		.to_string();
	...
	let emtk_framework_dll = env::current_exe()
		...
		.unwrap()
		.join("deps")
		.join("emtk_framework.dll")
		.display()
		.to_string();

	unsafe {
		crate::injector::inject(&emtk_framework_dll, &exanima_exe.display().to_string())?;
	}
