...
impl Instance {
	...
	pub fn update(&mut self, message: Message) -> Action {
		match message {
			...
			Message::PluginClosed => {
				...
				self.is_plugin_fullscreen = false;
			}
						...
						.map(move |Row { plugin_id, .. }| {
							...
							Task::done(Message::Loading)
								.chain(
									...
									.and_then(|buffer| {
										Task::done(Message::TextEditorLoaded(buffer))
									}),
								)
								.chain(Task::done(Message::Loaded))
						})
			...
			Message::TextEditorLoaded(buffer) => {
				...
				self.plugin = Plugin::Editor(content);
			}
			Message::TextEditorAction(action) => {
				if let Plugin::Editor(content) = &mut self.plugin {
	...
	fn plugin_controls(&self) -> Element<Message> {
		row![
			...
			.on_press(Message::PluginFullscreenToggled)
			.width(31)
			.height(31),
	...
	fn plugin_settings(&self) -> Element<Message> {
		...
		let Some(Row { plugin_id, .. }) = self.table.rows.get(row_index) else {
			return Space::new(Fill, Fill).into();
		};
